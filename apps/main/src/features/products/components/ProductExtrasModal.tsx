...
export const ProductExtrasModal = () => {
  ...
  const addItemToCart = () => {
    ...
    const getUniqueID = () => {
      ...
      return btoa(`${selectedModalProduct?.title},${selectedSize?.label},${sorted}`)
    }
  ...
  useEffect(() => {
    selectedModalProduct?.sizes && setSelectedSize(selectedModalProduct.sizes[0])
  ...
  return (
    <Modal isOpen={isExtrasModalOpen} onClose={closeExtrasModal}>
      <div className='flex w-full flex-col gap-2'>
        <h3 className='text-center text-xl font-semibold'>{selectedModalProduct?.title}</h3>
        <div className='flex flex-col flex-wrap items-start'>
          ...
          {/* extras dropdown */}
          {selectedModalProduct?.extras && selectedModalProduct.extras.length > 0 ? (
